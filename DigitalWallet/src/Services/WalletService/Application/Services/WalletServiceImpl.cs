...
public class WalletServiceImpl : IWalletQueryService, IWalletCommandService
{
    ...
    public async Task<TopUpResponseDto> TopUpAsync(Guid userId, TopUpRequestDto request)
    {
        ...
        try
        {
            ...
            if (tracker.TopUpTotal + request.Amount > _limits.DailyTopUpLimit)
                throw new InvalidOperationException($"Daily top-up limit of ₹{_limits.DailyTopUpLimit:N2} would be exceeded.");
            ...
            _logger.LogInformation("TopUp completed: {TopUpId} ₹{Amount} for wallet {WalletId}", topUp.Id, request.Amount, wallet.Id);

            await _bus.Publish(new TopUpCompleted
            {
                UserId         = userId,
                WalletId       = wallet.Id,
        ...
        catch
        {
            ...
            await _bus.Publish(new PaymentFailed { UserId = userId, WalletId = wallet.Id, TransactionId = Guid.Empty, Amount = request.Amount, Reason = "Top-up processing failed", TransactionType = "TopUp", OccurredAt = DateTime.UtcNow });
            throw;
        }
    }
    ...
    public async Task<TransferResponseDto> TransferAsync(Guid userId, TransferRequestDto request)
    {
        ...
        var receiverWallet = await _uow.WalletAccounts.FindByUserIdAsync(request.ToUserId)
        ...
        try
        {
            ...
            if (tracker.TransferTotal + request.Amount > _limits.DailyTransferLimit)
                throw new InvalidOperationException($"Daily transfer limit of ₹{_limits.DailyTransferLimit:N2} would be exceeded.");
            ...
            _logger.LogInformation("Transfer completed: {TransferId} ₹{Amount} from {From} to {To}", transfer.Id, request.Amount, senderWallet.Id, receiverWallet.Id);
        ...
        {
            await tx.RollbackAsync();
            await _bus.Publish(new PaymentFailed { UserId = userId, WalletId = senderWallet.Id, TransactionId = Guid.Empty, Amount = request.Amount, Reason = "Transfer processing failed", TransactionType = "Transfer", OccurredAt = DateTime.UtcNow });
            throw;
        }
    }
    ...
    /// <summary>Retrieves the user's wallet account or throws when no wallet exists.</summary>
    private async Task<WalletAccount> GetWalletOrThrow(Guid userId)
    {
        return await _uow.WalletAccounts.FindByUserIdAsync(userId)
