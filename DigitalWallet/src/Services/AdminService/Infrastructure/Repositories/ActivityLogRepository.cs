...
public class ActivityLogRepository : IActivityLogRepository
{
    ...
    public Task<int> CountActiveCampaignsAsync()
    {
        var now = DateTime.UtcNow;
        return _db.Campaigns.CountAsync(c => c.IsActive && c.StartsAt <= now && c.EndsAt >= now);
    }

    public Task<int> CountAllCampaignsAsync() =>
        _db.Campaigns.CountAsync();
